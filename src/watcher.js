...
const eventMap = {
  add: (...args) => syncer.add(...args),
  change: (...args) => syncer.add(...args),
  addDir:  (...args) => syncer.addDir(...args),
  unlink: (...args) => syncer.remove(...args),
  unlinkDir: (...args) => syncer.remove(...args),
};
...
const watchFolder = (watchPath, syncPath, isSource, key) => {
  ...
  chokidar.watch(watchPath, options).on('all', (event, data) => {
    ...
    eventMap[event](fullPath, watchPath, syncPath, isSource, key)
      .then(({ syncedPath }) => {
